...
class OpenVLAServer:
    ...
    def predict_action(self, payload: Dict[str, Any]) -> str:
        try:
            ...
            # image_primary = cv2.resize(image_full_original, (640, 480), interpolation=cv2.INTER_AREA)
            # image_wrist = cv2.resize(image_wrist_original, (640, 480), interpolation=cv2.INTER_AREA)
...
def output_action_string(action):
    output_str = "["
    for a in action:
        output_str += f"{a:.4f}, "
...
def deploy(cfg: Config) -> None:
    ...
    data_dict = np.load("./example_data/data.npy", allow_pickle=True).item()

    action_list = []

    info = {
        "images": data_dict["observation"]["rgb"][24],
        "instruction": "test instruction",
    }
    server.debug_cnt = 24
    actions = server.predict_action(info)
    print(output_action_string(actions[0]))

    # for i in range(1):
    #     # print("processing episode:", i)
    #     info = {
    #         "images": data_dict["observation"]["rgb"][i],
    #         "instruction": "test instruction",
    #     }
    #     actions = server.predict_action(info)
    #     print(output_action_string(actions[0]))
