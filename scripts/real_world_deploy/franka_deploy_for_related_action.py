...
@dataclass
class Config:
    name: str = "panda_chunk_rlds_dataset"
...
def change_to_delta_action(vla_action_chunk: np.ndarray) -> np.ndarray:
    """
    Convert related action representation (w.r.t. first frame) into delta action (between adjacent frames).
    - Input shape: (16, 7)
    - Output shape: (16, 7)
    - Format: [x, y, z, roll, pitch, yaw, gripper]
    """
    assert vla_action_chunk.shape == (16, 7), "Input must be of shape (16, 7)"
    
    delta_actions = []

    # First frame: keep as is (same as related action)
    delta_actions.append(vla_action_chunk[0])

    for i in range(1, 16):
        # Δposition = pos[i] - pos[i-1]
        delta_pos = vla_action_chunk[i, :3] - vla_action_chunk[i - 1, :3]

        # Δrotation = rot[i] * rot[i-1]^T
        rot_prev = euler2mat(*vla_action_chunk[i - 1, 3:6], axes='sxyz')
        rot_curr = euler2mat(*vla_action_chunk[i, 3:6], axes='sxyz')
        delta_rot = np.dot(rot_curr, rot_prev.T)
        delta_euler = mat2euler(delta_rot, axes='sxyz')
        delta_euler = np.array([0, 0, 0]) # for debuging

        # gripper: keep as absolute (same as related action)
        gripper = vla_action_chunk[i, 6]

        delta_action = np.concatenate([delta_pos, delta_euler, [gripper]])
        delta_actions.append(delta_action)

    return np.stack(delta_actions[1:], axis=0)
...
class OpenVLAServer:
    def __init__(self, cfg: Config) -> Path:
        ...
        if cfg.use_l1_regression or cfg.use_diffusion:
            action_head = get_action_head(cfg, 
                                          vla.llm_dim, 
                                          action_dim=112, 
    ...
    def create_vla_and_processor(self, cfg: Config):
        ...
        vla = get_vla(cfg, action_dim=112)
