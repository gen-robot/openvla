...
#SBATCH --job-name=pytorch_multi_node_torchrun_fixed # 作业名称
#SBATCH --nodes=2                              # 2个节点
...
#SBATCH --output=logs/multi_node_torchrun_fixed_%j.out
#SBATCH --error=logs/multi_node_torchrun_fixed_%j.err
...
export MASTER_PORT=39514
export NNODES=$SLURM_NNODES
export NPROC_PER_NODE=$SLURM_GPUS_PER_NODE

echo "=== Multi-Node Distributed Training Setup (torchrun fixed) ==="
...
echo "=== Starting Multi-Node Training (torchrun fixed) ==="

# 使用更简单的torchrun方法 - 不使用复杂的rendezvous
srun bash -c "
...
echo "Multi-node torchrun fixed job completed." 