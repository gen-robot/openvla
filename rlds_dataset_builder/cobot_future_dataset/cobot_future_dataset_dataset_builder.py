...
class CobotFutureDataset(tfds.core.GeneratorBasedBuilder):
    ...
    def _generate_examples(self, path) -> Iterator[Tuple[str, Any]]:
        ...
        def _parse_example(episode_path):
            ...
            episode = []
            for i in range(first_idx-1, num_episodes-1-25):
                ...
                episode.append({
                    ...
                    'action': process_qpos(f['observations']['qpos'], i+25).astype(np.float32), #process_action(f['action'], i).astype(np.float32),
                    'base_action': f['base_action'][i].astype(np.float32),
                    'instruction': instruction,
                    'terminate_episode': i == num_episodes - 1 - 25,
                })
