...
class PandaSimplerSftEncodeMPDataset(tfds.core.GeneratorBasedBuilder):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.tasks = [
            ...
            # "scp/PandaPutSpoonOnTableClothInRandomScene-v1/20250401_225725/data", # 450+50
    ...
    def _split_generators(self, dl_manager: tfds.download.DownloadManager):
        """Define data splits."""
        return {
            'train': self._generate_examples(1900, spare=100),
            'val': self._generate_examples(100, start=1900),
        }
    ...
    def _generate_examples(self, num_ep, spare=0, start=0) -> Iterator[Tuple[str, Any]]:
        ...
        def _parse_example(episode_path):
            ...
            for i in range(len(data["action"])):
                if data["is_image_encode"]:
                    image = np.array(cv2.imdecode(np.frombuffer(data["image"][i], np.uint8), cv2.IMREAD_COLOR))
                else:
