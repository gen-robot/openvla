...
class ExampleDataset(tfds.core.GeneratorBasedBuilder):
    ...
    def _split_generators(self, dl_manager: tfds.download.DownloadManager):
        """Define data splits."""
        return {
            'train': self._generate_examples(0, 25),
            'val': self._generate_examples(25, 3),
        }
    ...
    def _generate_examples(self, start_ep, num_ep) -> Iterator[Tuple[str, Any]]:
        ...
        for task in self.tasks:
            ...
            files = files[start_ep:start_ep + num_ep]

            assert len(files) == num_ep, f"Expected {num_ep} episodes, got {len(files)}"
