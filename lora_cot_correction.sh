...
correction_dataset_name="libero_correction_object_no_noops"
...
if [ ${cot_full} = True ]; then
  torchrun --standalone --nnodes 1 --nproc-per-node ${num_gpus} vla-scripts/finetune.py \
    ...
    --run_root_dir checkpoints/${correction_dataset_name} \
    ...
    --correction_dataset_name ${correction_dataset_name}
  #   --cot_tags ${cot_tags}
else
  torchrun --standalone --nnodes 1 --nproc-per-node ${num_gpus} vla-scripts/finetune.py \
    --vla_path ${vla_path} \
